...
class ModelSpace{
    public:
        ...
        void buildPartitions(int Z, int N, int M);
        ...
        int m_nSpStates;
        int m_nPartitions;
        double computeM(std::vector<int> partition);

        double SPEnergy(int spState, std::vector<double> spsEnergies);
        int orbital(int spState);
        bool brokenPairs(int partition);
private:
        void findSublists(std::vector<int> spStates, std::vector<int> bufferArray, std::vector<std::vector<int> > &configurationArray, int N, int nSpStates, int index, int chosen, int M);
};
